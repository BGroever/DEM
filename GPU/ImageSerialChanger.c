...
int main(){
    ...
    fclose(file);
    char const* const fileName = "Dem2016.txt";
    FILE* file = fopen(fileName, "r");
    char line[256];
    int d_dem[SIZE] = {0};
    char c_dem[SIZE][256] = {{'\0'}};
    int k_dem=0;
          ...
          for (int kk=0; kk<SIZE; kk++){
            if (d[kk]==nu){
              for(int ll=0; ll<SIZE; ll++){
                if(c_dem[ll]==c[kk]){
                  int bl_ = int(nu/65536);
                  int gr_ = int((nu-65536*bl)/256);
                  int re_ = int(nu-65536*bl-256*gr);
