...
void send_receive_save(int rank, int size, int* o, double* X, int startm, int startn, int endm, int endn, int m, int n, char* pngfile, char* outputfile){
  ...
  if (rank == 0){
      ...
      save_map(o, X,pngfile, outputfile);
  }
}
