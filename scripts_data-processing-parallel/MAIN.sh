...
then
    ...
    hadoop fs -put IHME_USA_COUNTY_CVD_MORTALITY_RATES_1980_2014_NATIONAL_Y2017M05D16.txt

    mkdir -p data_for_maps/$fldr

    spark-submit --num-executors 4 --executor-cores 2 $runFile

    dataYears=(1980 1985 1990 1995 2000 2005 2010 2014)

    for year in ${dataYears[@]}
        do
            hadoop fs -get data_$year.txt

            cat data_$year.txt/* > $dataFldr/year$year.txt

            rm -rf data_$year.txt/

            hadoop fs -rm -R -f data_$year.txt
    done
...
then
    ...
    hadoop fs -put Population-Weighted_Ultraviolet_Irradiance__2004-2015.csv
...
then
    ...
    hadoop fs -put Daily_County-Level_Ozone_Concentrations__2001-2014.csv
...
then
    ...
    hadoop fs -put Daily_Census_Tract-Level_PM2.5_Concentrations__2011-2014.csv
