...
conf = SparkConf().setMaster('local').setAppName('UVirradiance')
...
uvRDD = sc.textFile("raw_data/Population-Weighted_Ultraviolet_Irradiance__2004-2015.csv",1)
...
# to get ALL counties
join2015 = year2015.union(allCounties).groupByKey().filter(lambda x: (x[0] != rmCty[0]) and (x[0] != rmCty[1])).map(lambda x: str(str(yearMean(mean2015, list(x[1])[0])) + ' ' + x[0]))
#-------------------
